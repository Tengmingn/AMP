...
def parse_args():
    parser = argparse.ArgumentParser(description='DBFNet evaluation')
    ...
    def evaluate_folder(self, folder_path, gt_folder_path):
        for filename in os.listdir(folder_path):
            if not filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                continue
            gtname = filename[:4]+'_mask.png'
            pred_path = os.path.join(folder_path, filename)
            gt_path = os.path.join(gt_folder_path, gtname)
        ...
        accfilename = "DBF_acc" + folder_path[-11:-5]
        with open(accfilename, "w") as file:
            file.write('Model path is :')
            file.write(gt_folder_path)
