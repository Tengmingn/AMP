...
############################ user add #################################
    def user_forward_features(self, x, select_layers = list, masks=None):
        B, _,h, w = x.shape
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in select_layers:
                # 跳过CLS token和storage tokens，只取patch tokens
                cls_storage_tokens = x[:, :1 + self.n_storage_tokens, :]
                patch_tokens = x[:, 1 + self.n_storage_tokens:, :]
                outs.append(
                    patch_tokens.permute(0, 2, 1) # 跳过cls token，只取patch token
                    .view(B, -1, h // self.patch_size, w // self.patch_size).contiguous()
                )
                extra_outs.append(cls_storage_tokens) # 储存cls token 和 storage token

        return extra_outs,outs
    ######################################################################
